...
| `PAYMENT_GATEWAY_OPERATOR_ALERT_SINK_URL` | Runtime | Non-secret | No | Payment gateway only | Optional HTTP JSON endpoint for actionable operator alerts such as reconciliation exceptions. Delivery is non-blocking and fail-open. |
| `PAYMENT_GATEWAY_OPERATOR_ALERT_SINK_PATH` | Runtime | Non-secret path | No | Payment gateway only | Optional JSONL file path for actionable operator alerts. In self-hosted deployments this is mounted under `/app/alerts`. |
| `PAYMENT_GATEWAY_OPERATOR_ALERT_SINK_TIMEOUT_MS` | Runtime | Non-secret | No | Payment gateway only | HTTP operator alert sink timeout. Default `1500`. |
...
| `PAYMENT_GATEWAY_RECONCILIATION_SCHEDULE_ENABLED` | Runtime | Non-secret | No | Payment gateway only | Enables automatic scheduled reconciliation evidence exports. Default `false`. |
